#-------------------------------------------------------------------------------
# Append-Flush-Close Write class
#   Class for testing append-flush-close to disk: {open_append-write-flush-close-wait}...
#-------------------------------------------------------------------------------
...
#           -chunk_size:    number of bytes to be written to disk for each (open_append-write-flush-close) operation
#           -chunk_time:    wait time in seconds between (open_append-write-flush-close) operations
...
class Append():
    def __init__(self, fno, fsize, ftype, chunk_size=4096, chunk_time=0, itime=0, sanity_check=False, output_folder=CWD, machine_id=None, to_grafana=True):
        self.test_type      = "append"       # DO take care of modifying this when you write a new test
    ...
    def run(self):
        ...
        for i, cp in enumerate(payload):
            ...
            try:
                ...
                append(cp, fname, self.chunk_size, self.chunk_time)             # This is the actual write function
                write_end = time.time()
...
#       - fout: filename
#       - chunk_size: number of bytes to be written to disk for each (open_append-write-flush-close) cycle
#       - chunk_time: wait time in seconds between (open_append-write-flush-close) cycles
#-------------------------------------------------------------------------------
...
#opt = process_opt()
#
#current_test = Append(opt.file_no, opt.file_size, opt.file_type, opt.chunk_size, opt.chunk_time, opt.inter_time, opt.check_data, opt.out_dir, opt.machine_id, opt.to_grafana)
#current_test.run()
...
#CHUNK_TIME = 0.2       # Wait time in seconds between flush + os.fsync cycle
#
...
#
#    def __init__(self, fno,   fsize,      ftype,    chunk_size=4096, chunk_time=0, itime=0, sanity_check=False, output_folder=CWD, machine_id=None, to_grafana=True):
#current_test = Append(FILE_NO, FILE_SIZE, FILE_TYPE, CHUNK_SIZE, CHUNK_TIME, INTER_TIME, CHECK_DATA, OUTPUT_DIR, MACHINE_ID, TO_GRAFANA)
#current_test.run()
