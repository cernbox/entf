    ...
    def run(self):
        ...
        for i, cp in enumerate(payload):
            ...
            try:
                ...
                plain_write(cp, fname)      # This is the actual write function
                write_end = time.time()
...
#      - fout: filename
#-------------------------------------------------------------------------------
def plain_write(payload, fout):
    fwrite = open(fout, 'wb')
    fwrite.write(payload)
    fwrite.close()
    return
...
#opt = process_opt()
#
#current_test = PlainWrite(opt.file_no, opt.file_size, opt.file_type, opt.inter_time, opt.check_data, opt.out_dir, opt.machine_id, opt.to_grafana)
#current_test.run()
...
#
#    def __init__(self, fno, fsize, ftype, itime=0, sanity_check=True, output_folder=CWD, machine_id=None, to_grafana=True):
#current_test = PlainWrite(FILE_NO, FILE_SIZE, FILE_TYPE, INTER_TIME, CHECK_DATA, OUTPUT_DIR, MACHINE_ID, TO_GRAFANA)
#current_test.run()
